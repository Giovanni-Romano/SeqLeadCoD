...
#=========================================================================================
# SIMULATION 4 IMPUTATION 10%
#=========================================================================================

cat('Starting simulation set 4 imputation')
...
M.na.init = 1
k.init = K-1
gam = 0.2833003 #AntMAN::AM_find_gamma_Pois(n=(nj*K),Kstar=4,Lambda = 4)

for (seed in seed_set) {
  
  cat('Simulation 4 missing 10.',seed,'\n')
  ...
  #dropping values
  data_na = data_list_4[[seed]]$data*imputation_matrix
  
  #random values for beginning
  attributes_sizes = data_list_4[[1]]$attributes$attri_size
  ...
  for (i in 1:nrow(missing_pos)){
    ...
    #storing true_values
    true_values_tmp[i] = data_list_4[[seed]]$data[r,c]
  }
  ...
  set.seed(seed)
  start_time = Sys.time()
  sim_4[[seed]] = gibbs_mix_con_imput(G=10000,
                                burnin = 5000,
  ...
  imputed_data[[seed]] = sim_4[[seed]]$samp_data

}

cat('Saving outputs \n')

save.image(paste("output_4NA_",p.missing*100,".RData",sep=""))
