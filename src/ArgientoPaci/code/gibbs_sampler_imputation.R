...
gibbs_mix_con_imput = function(G,
                         thin       = 1,
                         burnin     = 0,
                         data_with_na,
                         missing_pos,  #matrix of size (number of missing values, 2), where each row has the row and column of the missing value in the "data_with_na"
                         u,
                         v,
                         Lambda     = 1,
                         gam        = 1,
  ...
  for (iter in 2:Iterations) {
    ...
    samp_data.curr = c()
    for (pos in 1:nrow(missing_pos)){
      row.tmp = missing_pos[pos,1]
      col.tmp = missing_pos[pos,2]
      current_cluster_allocation = C.curr[row.tmp]
      
      attri_prob = rep(NA,attri_size[col.tmp])
      
      #P(a_j=c_j)
      c_prob = prob.c_j(m_j=attri_size[col.tmp],
                        sigma_j=Sigma.curr[current_cluster_allocation,col.tmp])
      
      attri_prob[Cent.curr[current_cluster_allocation,col.tmp]] = c_prob
      
      #P(a_j≠c_j)
      attri_prob[is.na(attri_prob)]=(1-c_prob)/(attri_size[col.tmp]-1)
      
      #Sampling
      samp_data.curr[pos] = sample(attri_size[col.tmp],size=1,prob=attri_prob)
      data[row.tmp,col.tmp] = samp_data.curr[pos]
    }
    ...
    if(iter>=burnin& iter%%thin == 0){
      ...
      #Saving sampled data for missing value imputation
      Samp_data = rbind(Samp_data,samp_data.curr)
