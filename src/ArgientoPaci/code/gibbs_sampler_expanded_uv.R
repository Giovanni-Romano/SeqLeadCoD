...
gibbs_mix_con = function(G,
  ...
  for (iter in 2:Iterations) {
    ...
    ###############################
    Cent.curr = Sigma.curr.new = u_sigma.curr.new = v_sigma.curr.new= matrix(data=NA,nrow=M.curr,ncol = p)
     ...
     for (i in 1:k.curr) {
      ...
      for (j in 1:p) {
        ...
        Sigma.curr.new[i,j]=rhyper_sig(n = 1,
                                    d = v_sigma.curr[i,j]+dd,
                                    c = u_sigma.curr[i,j]+cc,
                                    m = attri_size[j])
        ### SAMPLING u AND v ###
        
        ### sampling u
        proposal_u = rnorm(1,
                           mean=u_sigma.curr[i,j],
                           sd=0.5)
        ...
        alpha_u = (dhyper_sig_raf(x = Sigma.curr.new[i,j],
                                  d = v_sigma.curr[i,j],
                                  c = proposal_u,
                                  m = attri_size[j],
                                  log_scale = T)+
                     dgamma(proposal_u,a1,b1,log = T))-
          (dhyper_sig_raf(x = Sigma.curr.new[i,j],
                          d = v_sigma.curr[i,j],
                          c = u_sigma.curr[i,j],
                          m = attri_size[j],
                          log_scale = T)+
             dgamma(u_sigma.curr[i,j],a1,b1,log = T))
        
        
        alpha_u <- min(0,alpha_u)
        test = log(runif(1))
        if(test<alpha_u){
          u_sigma.curr.new[i,j] = proposal_u
        }else{
          u_sigma.curr.new[i,j] = u_sigma.curr[i,j]
          
        }
        
        ### sampling v 
        proposal_v = rnorm(1,
                           mean=v_sigma.curr[i,j],
                           sd=0.5)
        if (proposal_v<=0){
          proposal_v = (-1)*proposal_v
        }
        
        alpha_v =  (dhyper_sig_raf(x = Sigma.curr.new[i,j],
                                   d = proposal_v,
                                   c = u_sigma.curr.new[i,j],
                                   m = attri_size[j],
                                   log_scale = T)+
          ...
          (dhyper_sig_raf(x = Sigma.curr.new[i,j],
                          ...
                          m = attri_size[j],
                          log_scale = T)+
             dgamma(v_sigma.curr[i,j],a2,b2,log = T))
    
        alpha_v = min(0,alpha_v)
        test = log(runif(1))
        if(test<alpha_v){
          v_sigma.curr.new[i,j] = proposal_v
        }else{
          v_sigma.curr.new[i,j] = v_sigma.curr[i,j]
        }
    ...
    for (i in M.na.vector) {
      if(M.na.curr==0){
        ...
        for (j in 1:p) {
          ...
          ### SAMPLING u AND v ###
          u_sigma.curr[i,j] = rgamma(1,a1,b2)
          v_sigma.curr[i,j] = rgamma(1,a2,b2)
