...
# saveRDS(cluster_sizes,
#         file = paste0(OUTFOLDER, "cluster_sizes_", SEED, ".RDS"))
...
cluster_info = 
  ...
  mutate(bottomy = cumsum(lag((Sizeprop), default = 0)),
         topy = cumsum(Sizeprop)) %>%
  ungroup()
...
ghe_PPE = ghe_PPE.init %>% rename(Cl.OG = Cl) %>% 
  left_join(cluster_info %>% select(Year, Cl.OG, Cl, Size), 
            by = c("Year", "Cl.OG"))
...
# saveRDS(mu_df, paste0(OUTFOLDER, "mu_", SEED, ".RDS"))

## Regional composition of each cluster ----
propregion_df = ghe_PPE %>% 
  select(Year, CountryN, RegionN, Cl, Size) %>% 
  rename(Size_cluster = Size) %>% 
  distinct() %>% 
  group_by(Year, Cl, RegionN, Size_cluster) %>% 
  summarise(Size_regionclust = n()) %>% 
  mutate(Prop_regionclust = Size_regionclust / Size_cluster) %>% 
  ungroup() %>% 
  select(Year:RegionN, Prop_regionclust, Size_regionclust, Size_cluster)

# saveRDS(propregion_df,
#         paste0(OUTFOLDER, "prop_region_", SEED, ".RDS"))
...
# saveRDS(strength_mu,
#         paste0(OUTFOLDER, "strengt_mu_", SEED, ".RDS"))
...
for (y in seq_along(years)){
  ...
  if (SAVE){
    if (!dir.exists(paste0(IMGFOLDER, "/heatmap_centers"))){dir.create(paste0(IMGFOLDER, "/heatmap_centers"))}
    
    ggsave(plot = plt_centers + 
             # Name of the cause within each tile
             geom_text(aes(x = (rightx+leftx)/2, y = -(bottomy+topy)/2, label = CauseS),
                       angle = 90, size = 2),
           filename = paste0("heatmap_centers/heatmap_", yyyy, ".pdf"),
           path = IMGFOLDER,
           height = 6, width = 9)
  }
  
  list_plot_centers[[y]] = plt_centers + theme(legend.position = "none")
}

# saveRDS(list_plot_centers,
#         file = paste0(OUTFOLDER, "list_plot_centers.RDS"))

# Create common legend for all years. To do so create not-printed plot with
# all years and clusters, to get all possible causes
...
ggsave(plot = plt_centers_together,
       filename = "estimated_centers.pdf",
       path = IMGFOLDER,
       width = 15, height = 12)


# Plot partition flow ----
plt_flow = plot_tHMM_flow(ppe = ghe_PPE %>% select(Year, Country, Cl) %>% distinct() %>% 
                            mutate(Cl = as.integer(as.character(Cl))) %>% 
                            pivot_wider(id_cols = Country, names_from = Year, values_from = "Cl") %>% 
                            column_to_rownames("Country") %>% as.matrix(), tosort = F)
...
df_flow = ghe_PPE %>% 
  select(Country, RegionN, Cl, Year) %>% distinct() %>% 
  mutate(freq = 1/ncountries,
         Year = as.integer(factor(Year)))

df_flow_summary <- df_flow %>% 
  group_by(Year, Cl, RegionN) %>%
...
plt_flow_regions = df_flow %>%
  ...
  geom_bar(data = df_flow_summary,
           mapping = aes(x = Year, y = n, fill = ID),
           color = "transparent",
           linewidth = 0.01,
           width = 0.335,
...
# palette <- ggsci::pal_npg("nrc", alpha = 1)(8)
# palette <- c(palette, ggsci::pal_nejm("default")(8))
palette_cluster = ggsci::pal_d3("category20")(20)[-8]
names(palette_cluster) = 1:length(palette_cluster)

source("src/utils.R")
PPE_seqsort = seqsort(PPE) %>% as.data.frame() %>% 
  rownames_to_column("CountryN") %>% 
  pivot_longer(-CountryN, names_to = "Year", values_to = "Cluster") %>% 
  left_join(ghe %>% select(CountryN, Country) %>% distinct(),
            by = "CountryN") %>% 
  rename(iso_a3 = Country)

map_data <- world %>%
  right_join(PPE_seqsort, by = c("iso_a3_eh" = "iso_a3"),
             relationship = "many-to-many")  # Match on ISO3 codes



plt_map = ggplot(map_data) +
  geom_sf(aes(fill = factor(Cluster))) +
  facet_wrap(~Year) +
  scale_fill_manual(values = palette_cluster, na.value = "white") +
...
if (SAVE) {if (!dir.exists(paste0(IMGFOLDER, "/networks"))){dir.create(paste0(IMGFOLDER, "/networks"))}}
...
for (t in 1:nyears){
  ...
  Z = Z[sort(names(Z))]
  Regions = Regions[sort(names(Regions))]
  
  t_standard <- table(Z, Regions)
  
  # define the colors in the pie-charts
  values <- list()
  for (h in 1:max(Z)){values[[h]] <- c(t_standard[h,])}
  pie_colors <- list()
  pie_colors[[1]] <- palette_regions
  ...
  l <- norm_coords(l, ymin=-0.85, ymax=0.85, xmin=-1.25, xmax=1.25)
  
  mark.g = list(); for(i in 1:max(Z)){mark.g[[i]] = i}
  
  pdf(file = paste0(IMGFOLDER, "/networks/network_", years[t], ".pdf"),
      width = 9, height = 6)
  ...
  plot(net_Y,
       ...
       mark.border = NA,
       mark.expand = (c(table(Z)))*0.6 + 5, mark.shape = 0.5)
  dev.off()
}


# Heatmap split by cluster ----
list_plot_data = list()
for (y in seq_along(years)){
  ...
  plt = ghe_PPE %>% 
    ...
    facet_grid(~Cl, scales = "free_x", space = "free") +
    scale_fill_manual(values = palette_causes) +
    theme(legend.position = "none",
          panel.grid = element_blank(),
  ...
  data_aux = unique(ghe_PPE %>% filter(Year == yyyy) %>% select(CountryN, Region, RegionN, Cl)) %>%
    cbind(., x = "Region")
  
  
  plt_aux = ggplot(data = data_aux,
                   mapping = aes(x = x, y = CountryN, fill = RegionN)) + 
    geom_tile() +
    scale_fill_manual("Region", values = palette_regions) +
    geom_text(aes(label = Region), size = 2, angle = 90) +
    facet_grid(~Cl, scales = "free_x", space = "free") +
    theme(legend.position = "none",
          panel.grid = element_blank(),
          plot.background = element_rect(fill = "white"),
  ...
  if (SAVE){
    if (!dir.exists(paste0(IMGFOLDER, "/heatmap"))){dir.create(paste0(IMGFOLDER, "/heatmap"))}
