...
cmline = c("female", 666) #commandArgs(trailingOnly = TRUE)
cat("Arguments from command line:", cmline, sep = " ")
sex = cmline[1]
seed = cmline[2] %>% as.integer

if (sex == "female"){
  ghe = readRDS("data/rds/GHEdf_female_5yrs_v2.RDS")
} else if (sex == "male"){
  ghe = readRDS("data/rds/GHEdf_male_5yrs_v2.RDS")
} else {
...
C_init = readRDS(paste0("output/tHMM/5yrs/PPE_", sex, ".RDS"))[[1]]
rownames(C_init) = rownames(ghe.array)
...
out = tHMM_gibbs_adaptive_PT(
  ...
  ctr_save = list(save = SAVE, filepath = paste0("output/tHMM/5yrs_V2/PT", sex, "/"),
