...
gibbs_mix_con = function(G,
  ...
  for (iter in 2:Iterations) {
    ...
    Cent.curr = Sigma.curr.new = matrix(data=NA,nrow=M.curr,ncol = p)
     ...
     for (i in 1:k.curr) {
      ...
      for (j in 1:p) {
        ...
        cc = nm-dd
       # cat(paste("iter",iter,"cluster", i,"variable",j,'dd',dd))
        #cat('\n')
        #cat(paste("iter",iter,"cluster", i,"variable",j,'cc',cc))
        #cat('\n')
        ### SAMPLING SIGMA ###
      ...
      if(M.na.curr==0){
        ...
        for (j in 1:p) {
          ...
          Cent.curr[i,j]= sample(attri_List[[j]],1,
                                 replace = T)
          ### SAMPLING SIGMA ###
          Sigma.curr[i,j]=rhyper_sig2(n = 1,
                                  d = v[j],
                                  c = u[j], 
                                  m = attri_size[j])
        }
    ...
    if(iter>=burnin & iter%%thin == 0){
      ...
      ## save.current
      # write.table(U.curr,"U.txt")
      # write.table(C.curr,"C.txt")
      # write.table(k.curr,"k.txt")
      # write.table(M.na.curr,"M.na.txt")
      # write.table(Cent.curr,"Cent.txt")
      # write.table(Sigma.curr,"Sigma.txt")
